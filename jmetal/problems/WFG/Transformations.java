...
public class Transformations {
  ...
  public float s_linear(float y, float A){
    return correct_to_01((float)Math.abs(y - A)/(float)Math.abs(Math.floor(A - y) + A));                
  } // s_linear
  ...
  public float s_multi(float y, int A, int B, float C){                
    ...
    tmp1 = ((float)4.0 * A + (float)2.0) *
           (float)Math.PI * 
           ((float)0.5 - (float)Math.abs(y - C)/((float)2.0 * ((float)Math.floor(C - y) + C)));
    tmp2 = (float)4.0 * B * 
           (float)StrictMath.pow((float)Math.abs(y - C)/((float)2.0 * ((float)Math.floor(C - y) + C))
                                 ,(float)2.0);
  ...
  public float b_param(float y, float u, float A, float B, float C){
    ...
    v = A - ((float)1.0 - (float)2.0 * u) * 
            (float)Math.abs((float)Math.floor((float)0.5 - u) + A);        
    exp = B + (C - B)*v;        
